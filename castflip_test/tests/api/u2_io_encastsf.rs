...
fn udata2() {
    ...
    let ne_size = ne_input.encastsf::<u32>(ne_slice_from_ne, NE).unwrap();
    let se_size = se_input.encastsf::<u32>(ne_slice_from_se, SE).unwrap();
    let le_size = le_input.encastsf::<u32>(ne_slice_from_le, LE).unwrap();
    let be_size = be_input.encastsf::<u32>(ne_slice_from_be, BE).unwrap();
