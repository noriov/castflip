How to utilize struct `std::io::Cursor` and a mutable fat pointer of
`&mut [u8]`

The examples below encast[^encast] a byte representation of the
UDP[^UDP] header as a value of struct `UdpHdr` then decast[^decast] a
value of struct `UdpHdr` as a byte representation of the UDP header.
