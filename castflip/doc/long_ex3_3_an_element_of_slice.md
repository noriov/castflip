...
big-endian as a value of the type in native-endian in two different
ways, then decasts a value of type `[[T; N]]` in native-endian as a
byte representation of the type in big-endian.
...
- Step 2: Method [`EncastMem::encastsf`] encasts a byte
  ...
  specified slice, and returns the number of the source bytes in
  [`Ok`]`(usize)`.
